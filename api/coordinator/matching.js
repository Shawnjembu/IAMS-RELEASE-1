...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const placementMap    = {};
      (placementsRes.data || []).forEach(p => { placementMap[p.student_id] = p; });
    ...
    if (req.method === "POST") {
      ...
      if (action === "auto") {
        ...
        const orgs       = (orgsRes.data || []).map(o => ({ id: o.id, extra: orgExtraMap[o.id] || null }));

        const assigned = [];
        for (const s of unassigned) {
          const extra = studentExtraMap[s.id] || null;
          const best  = orgs
            .map(o => ({ org_id: o.id, score: computeMatchScore({ extra }, o.extra || {}) }))
      ...
      const [sExtra, oExtra] = await Promise.all([
        sb.from("student_profiles").select("skills, preferred_location, program").eq("id", student_id).single(),
        sb.from("organization_profiles").select("industry, location").eq("id", org_id).single(),
      ]);
