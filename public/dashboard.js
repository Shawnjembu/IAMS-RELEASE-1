...
(async function () {
  ...
  if (!profile) {
    for (var attempt = 0; attempt < 3 && !profile; attempt++) {
      ...
      try {
        ...
        var json = await rr.json().catch(function(){ return { ok:false, error:"Invalid server response" }; });
        if (rr.status === 401) {
  ...
  var subtitle = roleLabel + " · " + _esc(session.user.email);
  if (role === "industrial_supervisor" && extra && extra.org_name) subtitle += " · " + _esc(extra.org_name);
  document.getElementById("dash-sub").textContent = subtitle;
  var hero = document.getElementById("dashboard-hero");
  if (hero) {
    hero.classList.add(role === "student" ? "hero-student" : role === "organization" ? "hero-organization" : role === "coordinator" ? "hero-coordinator" : role === "industrial_supervisor" ? "hero-industrial" : role === "university_supervisor" ? "hero-university" : "hero-student");
  }
  var roleTitle = document.getElementById("role-title");
  var roleDesc  = document.getElementById("role-desc");
  if (roleTitle) roleTitle.textContent = roleLabel;
  if (roleDesc)  roleDesc.textContent  = _roleDescription(role);
  _renderDashboardAvatar("sidebar-avatar", profile);
  _renderDashboardAvatar("nav-avatar", profile);
  var sidebarNameEl = document.getElementById("sidebar-name");
  var sidebarRoleEl = document.getElementById("sidebar-role-label");
  var sidebarEmailEl = document.getElementById("sidebar-email");
  var navNameEl = document.getElementById("nav-user-name");
  if (sidebarNameEl) sidebarNameEl.textContent = (profile && (profile.full_name || (extra && extra.org_name))) || session.user.email;
  if (sidebarEmailEl) sidebarEmailEl.textContent = session.user.email;
  if (sidebarRoleEl) sidebarRoleEl.textContent = roleLabel;
  if (navNameEl) navNameEl.textContent = ((profile && (profile.full_name || extra && extra.org_name)) || session.user.email || "").split(" ")[0];
  var ddProfile = document.getElementById("dropdown-profile-link");
  if (ddProfile) ddProfile.href = _profileHref(role);
  if (role === "industrial_supervisor" && extra && extra.org_name) {
    if (roleDesc) roleDesc.textContent = _roleDescription(role) + " Organisation: " + extra.org_name + ".";
    var dashSub = document.getElementById("dash-sub");
    if (dashSub) dashSub.textContent = roleLabel + " · " + _esc(session.user.email) + " · " + _esc(extra.org_name);
  }
...
async function _initOrgSupervisors(token) {
  ...
  async function loadSupervisors() {
    ...
    try {
      ...
      if (sups.length === 0 && invites.length === 0) {
        listEl.innerHTML = '<div class="text-gray-500 text-sm" style="padding:12px 0">No industrial supervisors invited yet.</div>';
        return;
      }
      window._orgActiveSupervisors = sups || [];
      var cards = [];
      sups.forEach(function (s) {
        cards.push('<div class="clean-list-card">' +
          '<div class="flex-between" style="align-items:flex-start"><div><div style="font-weight:800">' + _esc(s.full_name || '—') + '</div><div class="small muted">' + _esc(s.email) + (s.department ? ' · ' + _esc(s.department) : '') + '</div></div>' +
          '<span class="badge-role organization">Active</span></div></div>');
      });
      invites.forEach(function (i) {
        var actions = i.invite_link ? '<button class="btn btn-secondary btn-sm" data-copy-invite="' + _esc(i.invite_link) + '"><i class="fas fa-copy"></i> Copy Invite</button>' : '';
        cards.push('<div class="clean-list-card" style="background:#fcfcff">' +
          '<div class="flex-between" style="align-items:flex-start;gap:14px"><div><div style="font-weight:800">' + _esc(i.full_name || 'Pending invite') + '</div><div class="small muted">' + _esc(i.email) + '</div><div class="small muted" style="margin-top:6px">Status: ' + _esc(i.status) + '</div></div>' +
          '<div class="actions"><span class="badge-role student">Pending</span>' + actions + '</div></div></div>');
      });
      listEl.innerHTML = cards.join('');
      listEl.querySelectorAll('[data-copy-invite]').forEach(function(btn){
        btn.addEventListener('click', function(){
          navigator.clipboard.writeText(btn.getAttribute('data-copy-invite')).then(function(){ UI.showToast('success','Invite link copied.'); });
        });
      });
      if (window._lastOrgStudents) _populateOrgStudentsTable(window._lastOrgStudents);
    } catch (err) {
  ...
  if (createBtn) {
    createBtn.addEventListener("click", async function() {
      ...
      try {
        ...
        var d = await resp.json();
        if (!d.ok) throw new Error(d.error);
        showAlert("success", "Industrial supervisor invite created for " + email.trim() + ". Use the Copy Invite button from the list if email sending is not configured.");
...
function _populateOrgStudentsTable(students) {
  window._lastOrgStudents = students || [];
  var tbody = document.getElementById("org-students-body");
  if (!tbody) return;

  if (!students || students.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" class="text-center py-8 text-gray-500">No students attached yet. Students will appear here after they choose your organisation or after coordinator placement.</td></tr>';
    return;
  }

  var sups = window._orgActiveSupervisors || [];
  function supSelect(s) {
    if (!sups.length) return '<span class="text-gray-500 text-sm">Invite/activate supervisor first</span>';
    var options = '<option value="">Select supervisor…</option>' + sups.map(function(sp){
      return '<option value="' + _esc(sp.id) + '" ' + (sp.id === s.industrial_supervisor_id ? 'selected' : '') + '>' + _esc(sp.full_name || sp.email) + '</option>';
    }).join('');
    return '<div class="assign-supervisor-row">' +
      '<select class="border border-gray-300 rounded-lg px-3 py-2 text-sm" data-sup-select="' + _esc(s.placement_id) + '">' + options + '</select>' +
      '<button class="btn btn-primary btn-sm" data-assign-sup="' + _esc(s.placement_id) + '" data-student="' + _esc(s.student_id) + '"><i class="fas fa-user-check"></i> Assign</button>' +
      '</div>';
  }

  tbody.innerHTML = students.map(function(s) {
    var supName = s.industrial_supervisor ? (s.industrial_supervisor.full_name || s.industrial_supervisor.email) : '';
    return `
      <tr class="hover:bg-gray-50 border-b">
        ...
        <td class="px-4 py-4">
          ${supName ? '<div class="mb-2"><span class="status-pill success">' + _esc(supName) + '</span></div>' : '<div class="mb-2"><span class="status-pill neutral">Not assigned</span></div>'}
          ${supSelect(s)}
        </td>
      </tr>`;
  }).join("");

  tbody.querySelectorAll('[data-assign-sup]').forEach(function(btn){
    btn.addEventListener('click', async function(){
      var placementId = btn.getAttribute('data-assign-sup');
      var studentId = btn.getAttribute('data-student');
      var sel = tbody.querySelector('[data-sup-select="' + placementId + '"]');
      var supervisorId = sel ? sel.value : '';
      if (!supervisorId) { UI.showToast('error', 'Please select an active industrial supervisor.'); return; }
      ...
      try {
        ...
        var resp = await fetch('/api/supervisor/assign', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
          body: JSON.stringify({ placement_id: placementId, student_id: studentId, industrial_supervisor_id: supervisorId })
        });
        var data = await resp.json();
        if (!data.ok) throw new Error(data.error || 'Could not assign supervisor');
        UI.showToast('success', 'Student assigned to industrial supervisor. Their student list will now update.');
        // reload organisation students
        var r = await fetch('/api/organization/students', { headers: { Authorization: 'Bearer ' + token } });
        var d = await r.json();
        if (d.ok) _populateOrgStudentsTable(d.students || []);
      } catch (err) {
...
function _populateAvailableOrgs(orgs) {
  ...
  var filtered = (orgs || []).filter(function(o){ return o.can_accept !== false; });
  ...
  container.innerHTML = filtered.map(function(o) {
    ...
    var slots  = (o.extra && o.extra.slots != null) ? Number(o.extra.slots) : 0;
    var available = o.available_slots != null ? o.available_slots : (slots > 0 ? slots : 'Open');
    return '<div class="p-4 border-b hover:bg-gray-50">' +
      '<div class="flex justify-between items-start gap-4">' +
        '<div class="min-w-0">' +
          '<div class="font-semibold">' + _esc(name) + '</div>' +
          '<div class="text-sm text-gray-600">' + _esc(skills) + '</div>' +
          '<div class="text-xs text-gray-500 mt-1">Location: ' + _esc(loc) + '</div>' +
        '</div>' +
        '<div class="text-right">' +
          '<span class="px-3 py-1 bg-blue-200 text-xs rounded-full">' + (slots > 0 ? ('Available ' + _esc(String(available)) + ' / ' + _esc(String(slots))) : 'Open slots') + '</span>' +
        '</div>' +
      '</div>' +
    '</div>';
  }).join('') || '<p class="text-gray-500 text-center py-12">No organisations with available slots</p>';
}
...
async function _loadSupervisorStats(token, role) {
  try {
    ...
    var orgCard = document.getElementById('supervisor-org-card');
    var orgNameEl = document.getElementById('supervisor-org-name');
    var orgSubEl = document.getElementById('supervisor-org-sub');
    var orgInfo = d.supervisor && d.supervisor.organization ? d.supervisor.organization : null;
    if (role === 'industrial_supervisor' && orgInfo && orgCard) {
      orgCard.style.display = '';
      if (orgNameEl) orgNameEl.textContent = orgInfo.org_name || orgInfo.email || 'Organisation';
      if (orgSubEl) orgSubEl.textContent = [orgInfo.location, orgInfo.email].filter(Boolean).join(' · ') || 'Your workplace organisation';
    } else if (orgCard) {
...
async function _loadDeadlines(token, role) {
  try {
    // Final workflow rule: deadlines are student-facing on dashboards.
    // Supervisors create/manage deadlines from the Deadlines page, while
    // coordinator/organisation dashboards only monitor workflow status.
    ...
    var deadlineCard = list ? list.closest(".bg-white, .card") : null;
    ...
    if (!r.ok) return;
    var d = await r.json();
    var deadlines = (d.ok && d.deadlines) ? d.deadlines : [];
    if (countEl) countEl.textContent = deadlines.length;
    if (!list) return;
    if (!deadlines.length) {
      list.innerHTML = '<div class="text-gray-500 text-sm py-3">No upcoming deadlines for your account yet.</div>';
      return;
    }

    list.innerHTML = deadlines.map(function(dl) {
      return (
    ...
    // Count already updated above.
  } catch (_) {}
}


function _profileHref(role) {
  return ({
...
function _roleDescription(role) {
  return ({
    student: "Complete your profile, view your placement, submit logbooks and final report, and track supervisor feedback.",
    organization: "Host students, manage placement slots, invite industrial supervisors, and monitor attached interns.",
    coordinator: "Manage the attachment workflow: match students to organisations, monitor supervisor assignment, deadlines, and system issues. Coordinators do not assess students.",
    industrial_supervisor: "Supervise workplace performance and provide workplace feedback for assigned students. Academic reports and logbooks are reviewed by the assigned university supervisor.",
    university_supervisor: "Monitor academic progress, conduct visits, and submit visit evaluations for assigned students."
  })[role] || "Use the system according to your assigned role.";
}

function _renderDashboardAvatar(id, profile) {
  var el = document.getElementById(id);
  if (!el) return;
  var name = profile && (profile.full_name || profile.email) ? (profile.full_name || profile.email) : 'User';
  if (profile && profile.avatar_url) {
    el.innerHTML = '<img src="' + _esc(profile.avatar_url) + '" alt="avatar" style="width:100%;height:100%;object-fit:cover"/>';
  } else {
    var initials = String(name).trim().split(/\s+/).slice(0,2).map(function(x){ return x.charAt(0); }).join('').toUpperCase() || 'U';
    el.innerHTML = '<span class="text-white font-bold">' + initials + '</span>';
    if (id === 'nav-avatar') el.innerHTML = '<span class="text-primary font-bold text-xs">' + initials + '</span>';
  }
}
